...
const tools: Tool[] = [
  ...
  },
  {
    name: "get_feedback",
    description:
      "Retrieve feedback data from the fact_feedback table. Use for questions like 'What was the feedback on [resort] in [month]?', 'Show me feedback for [resort]', 'What did customers say about [resort]?'. Use ONLY when user wants to SEE feedback records (not count them). NEVER use for counting - always use analyze_data for counts. When user asks for 'all' records, omit the limit parameter. When user asks for a specific number, set limit to that number. Column names: 'log_date' (NOT 'feedback_date') for date queries, 'resort_name_fk' for resort name (use 'ilike' operator for case-insensitive matching), 'member_id_fk' for member, 'nps_score' for NPS score, 'csat_score' for CSAT score, 'sentiment' for sentiment, 'details_text' for feedback text. For questions like 'What was the feedback on Assonora in September 2025': Use get_feedback with filters: {'resort_name_fk': {'operator': 'ilike', 'value': 'Assonora'}, 'log_date': {'gte': '2025-09-01', 'lte': '2025-09-30'}}. IMPORTANT: Execute queries directly without showing your thinking process or step-by-step reasoning. Provide concise responses with only the results.",
    inputSchema: {
      type: "object",
      properties: {
        limit: {
